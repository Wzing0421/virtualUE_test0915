...
void MainWindow::on_start_clicked()
{
	...
	if(testversion)
	{
        ...
        num=sendSocket->writeDatagram((char*)sc2_regMsg,sizeof(sc2_regMsg),Ancaddr,regsendPort);//num返回成功发送的字节数量
	}
	else
	{
	    regtimer->start(5000);
	    num=sendSocket->writeDatagram((char*)regMsg,sizeof(regMsg),Ancaddr,regsendPort);//num返回成功发送的字节数量
	}
...
void MainWindow::recvRegInfo(){
    while(regUdpSocket->hasPendingDatagrams()){
        ...
        if(judge == 0x02 && registerstate == UNREGISTERED){//说明是authorization command
			...
			if (testversion)
			{
			    short len = htons(sizeof(regMsg_au));
                memcpy(SC2_header+6,(char*)&len,2);
                memcpy(sc2_regMsg_au,SC2_header,sizeof(SC2_header));
                memcpy(sc2_regMsg_au+sizeof(SC2_header), regMsg_au, sizeof(regMsg_au));
                num=sendSocket->writeDatagram((char*)sc2_regMsg_au,sizeof(regMsg_au),Ancaddr,regsendPort);//num返回成功发送的字节数量
			}
			else
			    num=sendSocket->writeDatagram((char*)regMsg_au,sizeof(regMsg_au),Ancaddr,regsendPort);//num返回成功发送的字节数量
            ...
            if (testversion)
            {
                ...
                memcpy(SC2_header+6,(char*)&len,2);
                memcpy(sc2_voiceDeRegisterRsp,SC2_header,sizeof(SC2_header));
                memcpy(sc2_voiceDeRegisterRsp+sizeof(SC2_header), voiceDeRegisterRsp, sizeof(voiceDeRegisterRsp));
                num=sendSocket->writeDatagram((char*)sc2_voiceDeRegisterRsp,sizeof(sc2_voiceDeRegisterRsp),Ancaddr,regsendPort);//num返回成功发送的字节数量
            }
            else
                num=sendSocket->writeDatagram((char*)voiceDeRegisterRsp,sizeof(voiceDeRegisterRsp),Ancaddr,regsendPort);//num返回成功发送的字节数量
            ...
            qDebug()<<"收到call setup";
			if (testversion)
                qDebug("recvbuf sc_call_id is 0x%02x, 0x%02x, 0x%02x, 0x%02x", *(recvbuf+3), *(recvbuf+4), *(recvbuf+5), *(recvbuf+6));
            ...
            if (testversion)
            {
			    len = htons(sizeof(callSetupAck));
                memcpy(SC2_header+6,(char*)&len,2);
                memcpy(sc2_callSetupAck,SC2_header,sizeof(SC2_header));
                memcpy(sc2_callSetupAck+sizeof(SC2_header), callSetupAck, sizeof(callSetupAck));
                num=sendSocket->writeDatagram((char*)sc2_callSetupAck,sizeof(sc2_callSetupAck),Ancaddr,regsendPort);
			}
			else
			    num=sendSocket->writeDatagram((char*)callSetupAck,sizeof(callSetupAck),Ancaddr,regsendPort);
            qDebug()<<"发送call setup ack，长度为 "<<num<<" 字节";
            callstate = U9;//呼叫确认态

            if (testversion)
            {
			    len = htons(sizeof(callAllerting));
                memcpy(SC2_header+6,(char*)&len,2);
                memcpy(sc2_callAllerting,SC2_header,sizeof(SC2_header));
                memcpy(sc2_callAllerting+sizeof(SC2_header), callAllerting, sizeof(callAllerting));
                num=sendSocket->writeDatagram((char*)sc2_callAllerting,sizeof(sc2_callAllerting),Ancaddr,regsendPort);
		    }
            else
            ...
            if (testversion)
            {
                ...
                num=sendSocket->writeDatagram((char*)sc2_callConnectAck,sizeof(sc2_callConnectAck),Ancaddr,regsendPort);//num返回成功发送的字节数量
            }
            else
                num=sendSocket->writeDatagram((char*)callConnectAck,sizeof(callConnectAck),Ancaddr,regsendPort);//num返回成功发送的字节数量
            ...
            if (testversion)
            {
                ...
                num=sendSocket->writeDatagram((char*)sc2_callReleaseRsp,sizeof(sc2_callReleaseRsp),Ancaddr,regsendPort);//发送callReleaseRsp
			}
			else
			    num=sendSocket->writeDatagram((char*)callReleaseRsp,sizeof(callReleaseRsp),Ancaddr,regsendPort);//发送callReleaseRsp
...
void MainWindow::init_voiceDeRegisterReq(){//初始化DeRegisterReq
    ...
    //STMSI
    voiceDeRegisterReq[3] = regMsg[3];
    voiceDeRegisterReq[4] = regMsg[4];
    voiceDeRegisterReq[5] = regMsg[5];
    voiceDeRegisterReq[6] = regMsg[6];
    voiceDeRegisterReq[7] = regMsg[7];
...
void MainWindow::init_voiceDeRegisterRsp(){
    ...
    //后面5个字节的STMSI
    voiceDeRegisterRsp[3] = regMsg[3];
    voiceDeRegisterRsp[4] = regMsg[4];
    voiceDeRegisterRsp[5] = regMsg[5];
    voiceDeRegisterRsp[6] = regMsg[6];
    voiceDeRegisterRsp[7] = regMsg[7];
    /*
    short len = htons(sizeof(voiceDeRegisterRsp));
...
void MainWindow::init_callSetup(){
    ...
    //STMSI
    callSetup[7] = regMsg[3];
    callSetup[8] = regMsg[4];
    callSetup[9] = regMsg[5];
    callSetup[10] = regMsg[6];
    callSetup[11] = regMsg[7];
...
void MainWindow::on_DeReigster_clicked()
{
    ...
    if (testversion)
    {
        short len = htons(sizeof(voiceDeRegisterReq));
        memcpy(SC2_header+6,(char*)&len,2);
        memcpy(sc2_voiceDeRegisterReq,SC2_header,sizeof(SC2_header));
        memcpy(sc2_voiceDeRegisterReq+sizeof(SC2_header), voiceDeRegisterReq, sizeof(voiceDeRegisterReq));
        num=sendSocket->writeDatagram((char*)sc2_voiceDeRegisterReq,sizeof(sc2_voiceDeRegisterReq),Ancaddr,regsendPort);//num返回成功发送的字节数量
    }
    else
        num=sendSocket->writeDatagram((char*)voiceDeRegisterReq,sizeof(voiceDeRegisterReq),Ancaddr,regsendPort);//num返回成功发送的字节数量
...
void MainWindow::on_call_clicked()//呼叫按钮
{
    ...
    //if (bUE4)
    //    calledBCDNumber = "19922130245";
    //else
    //    calledBCDNumber = "19922130244";
    ...
    if (testversion)
	{
		...
		num=sendSocket->writeDatagram((char*)sc2_callSetup,sizeof(sc2_callSetup),Ancaddr,regsendPort);//num返回成功发送的字节数量
	}
    else
	    num=sendSocket->writeDatagram((char*)callSetup,sizeof(callSetup),Ancaddr,regsendPort);//num返回成功发送的字节数量
...
void MainWindow::on_disconnect_clicked()//主叫或者被叫的结束通话按钮
{
    ...
    if(callstate == U4){//这种情况是在响铃的时候主叫点击了disconnect按钮
        ...
        if (testversion)
        {
            short len = htons(sizeof(callDisconnect));
            memcpy(SC2_header+6,(char*)&len,2);
            memcpy(sc2_callDisconnect,SC2_header,sizeof(SC2_header));
            memcpy(sc2_callDisconnect+sizeof(SC2_header), callDisconnect, sizeof(callDisconnect));
            sendSocket->writeDatagram((char*)sc2_callDisconnect,sizeof(sc2_callDisconnect),Ancaddr,regsendPort);
        }
        else
            sendSocket->writeDatagram((char*)callDisconnect,sizeof(callDisconnect),Ancaddr,regsendPort);
        qDebug()<<"主叫在响铃期间停止呼叫，发送call Disconnect(CANCEL)!";
        ...
        if (testversion)
        {
            ...
            sendSocket->writeDatagram((char*)callDisconnect,sizeof(callDisconnect),Ancaddr,regsendPort);
        qDebug()<<"UE 正常用户挂机，发送call Disconnect(BYE)!";
    }
...
void MainWindow::on_connect_clicked()//UE被叫的时候点击接听按钮
{
    ...
    if (testversion)
    {
        short len = htons(sizeof(callConnect));
        memcpy(SC2_header+6,(char*)&len,2);
        memcpy(sc2_callConnect,SC2_header,sizeof(SC2_header));
        memcpy(sc2_callConnect+sizeof(SC2_header), callConnect, sizeof(callConnect));
        num=sendSocket->writeDatagram((char*)sc2_callConnect,sizeof(sc2_callConnect),Ancaddr,regsendPort);
    }
    else
        num=sendSocket->writeDatagram((char*)callConnect,sizeof(callConnect),Ancaddr,regsendPort);
